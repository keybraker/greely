...
int main(int argc, char** argv){
    ...
    FILE* f1 = fopen ("wordDatabaseSource/wordSyllabusGr.txt", "r");
    if(!f1){ printf("ERROR f1"); exit(1); }
    FILE* f2 = fopen ("wordDatabaseFinal/wordSyllabusGr2.txt", "w+");
    if(!f2){ printf("ERROR f2"); exit(1); }
    FILE* f3 = fopen ("wordDatabaseFinal/wordSyllabusGr3.txt", "w+");
    ...
    while (fgets(buffer,255, f1)!= NULL)    { 
        ...
        // IDIAITEROTHTES
        if(strcmp(word, "από") == 0){ // προθεση θελει φτιαξημο
