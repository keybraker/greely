...
int main(int argc, char** argv){
    ...
    int jk = 0;
    wholeFile = calloc(2048/*strlen(buffer)+1*/, sizeof(char*));
    while (fgets(buffer,2048, f1)!= NULL)    { 
        // oti nanai prosoxi sti calloc kai realloc
        wholeFile = realloc(wholeFile, 2048*10/*strlen(buffer)+1*/* sizeof(char*));
        if(wholeFile[jk] == NULL){
            wholeFile[jk] = calloc(2*2048/*strlen(buffer)+1*/, sizeof(char));
